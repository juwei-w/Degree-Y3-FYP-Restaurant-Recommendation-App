...
class _ProfileScreenState extends State<ProfileScreen> with WidgetsBindingObserver {
  ...
  Future<User?> _handleSessionExpiry({bool showMessages = true}) async {
    ...
    if (currentUser == null && mounted && showMessages) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No active user session found. Please log in.')),
      );
    }
    return currentUser;
  }
  ...
  void _loadUserData() async {
    ...
    String? currentAuthEmail = refreshedUser?.email ?? FirebaseAuth.instance.currentUser?.email;

    if (refreshedUser == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Session expired or token invalid. Please log in again.')),
        );
      }
  ...
  Future<void> _sendVerificationToNewEmail() async {
    ...
    final String currentActualAuthEmail = FirebaseAuth.instance.currentUser?.email ?? _initialEmail;

    if (newEmailCandidate.isEmpty || newEmailCandidate == currentActualAuthEmail) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter a new email address different from your current one.')),
      );
      return;
    }
    bool isValidEmail = RegExp(r"^[a-zA-Z0-9.a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~]+@[a-zA-Z0-9]+\.[a-zA-Z]+").hasMatch(newEmailCandidate);
    if (!isValidEmail) {
    ...
    if (currentUser.email == newEmailCandidate) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('This is already your verified email address.')),
        );
        setState(() {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('We\'ve sent a verification email to $newEmailCandidate. After verifying, press Update to complete the email change.')),
        );
        setState(() {
      ...
      if (mounted) {
        ...
        if (e.code == 'requires-recent-login') {
          message = "This operation requires you to log in again for security reasons.";
  ...
  Future<void> _updateUserData() async {
    ...
    if (_newEmailVerificationSent && _emailController.text.trim() == _emailForWhichVerificationWasSent) {
      ...
      try {
        ...
        if (refreshedEmail == _emailForWhichVerificationWasSent) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Email successfully updated to $refreshedEmail. Please log in again.')),
            );
          }
          await FirebaseAuth.instance.signOut();
          if (mounted) {
        ...
        if (e.code == 'user-token-expired' || e.code == 'user-disabled' || e.code == 'user-not-found') {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Email successfully updated. Please log in again.')),
            );
          }
          await FirebaseAuth.instance.signOut();
          if (mounted) {
    ...
    if (newPassword.isNotEmpty) {
      try {
        ...
        if (e.code == 'requires-recent-login') {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Please log in again to update your password.')),
          );
          await FirebaseAuth.instance.signOut();
          if (mounted) {
            Navigator.of(context).pushNamedAndRemoveUntil('/login', (route) => false);
  ...
  Widget build(BuildContext context) {
    ...
    Color verifyButtonBackgroundColor = Theme.of(context).primaryColor.withOpacity(0.1); // Default background
    ...
    if (emailInTextField.isNotEmpty && emailInTextField == currentActualAuthEmail) {
    ...
    } else if (emailInTextField.isNotEmpty && emailInTextField != currentActualAuthEmail) {
      showVerifyButton = true;
      if (_newEmailVerificationSent && emailInTextField == _emailForWhichVerificationWasSent) {
        ...
        verifyButtonColor = const Color(0xFFFF7F59); // Matching figma button color
                      ...
                      Container(
                        ...
                        child: TextField(
                          ...
                          decoration: const InputDecoration(
                           ...
                           onChanged: (value) { // To update the displayed name above dynamically
                            if(mounted) setState(() {});
                          },
                        ),
                      ),
                        ...
                        children: [
                                ...
                                onChanged: (value) {
                                  if (mounted) {
                                    setState(() {
                                      ...
                                      if (text != _emailForWhichVerificationWasSent) {
                                        _newEmailVerificationSent = false; 
                                      }
                                    });
                                  }
                          ...
                          if (showVerifyButton)
                            Padding(
                              ...
                              child: TextButton(
                                onPressed: verifyButtonEnabled ? _sendVerificationToNewEmail : null,
